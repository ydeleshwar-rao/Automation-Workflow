...
function RecordTree({ record, recordKey, search, accent }: RecordTreeProps) {
  ...
  const entries = Object.entries(record).filter(
    ([key]) =>
      key.toLowerCase().includes(search.toLowerCase()) && !key.startsWith("_")
  );
...
interface ServiceM8TestStepProps {
...
export function ServiceM8TestStep({
  data,
  testStatus = "idle",
  isTrigger = false,
  onTestTrigger,
}: ServiceM8TestStepProps) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="ml-2">
            <h4 className="text-sm font-bold text-foreground">
              {isTrigger ? "Test ServiceM8 Trigger" : "Create Job in ServiceM8"}
            </h4>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              {isTrigger
                ? 'Click "Test trigger" to fetch sample record(s) from ServiceM8.'
                : "Testing the integration will create a job with the following information:"}
            </p>
          </div>
        </div>
        ...
        {testStatus === "testing" && (
          <div className="flex items-center gap-3 p-3 bg-primary/10 border border-primary/20 rounded-xl animate-pulse">
            ...
            <span className="text-[13px] font-bold text-primary">
              {isTrigger ? "Fetching from ServiceM8..." : "Connecting to ServiceM8..."}
            </span>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 p-3 bg-success/10 border border-success/20 rounded-xl">
            ...
            <span className="text-[13px] font-bold text-success">
              {isTrigger
                ...
                : "Success! Job created in ServiceM8."}
            </span>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-destructive/10 border border-destructive/20 rounded-xl">
              ...
              <span className="text-[13px] font-bold text-destructive">
                {isTrigger
                  ? "No records found. Make sure there is data in ServiceM8 for this event."
