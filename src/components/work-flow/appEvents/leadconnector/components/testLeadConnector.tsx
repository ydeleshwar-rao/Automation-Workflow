...
interface LeadConnectorTestStepProps {
...
export function LeadConnectorTestStep({
  data,
  testStatus = "idle",
  isTrigger = false,
  onTestTrigger,
}: LeadConnectorTestStepProps) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="ml-2">
            <h4 className="text-sm font-bold text-foreground">
              {isTrigger ? "Test LeadConnector Trigger" : "Create Contact in LeadConnector"}
            </h4>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              {isTrigger
                ? 'Click "Test trigger" to fetch sample record(s) from LeadConnector.'
                : "Testing the integration will create a contact with the following information:"}
            </p>
          </div>
        </div>
        ...
        {testStatus === "testing" && (
          <div className="flex items-center gap-3 p-3 bg-primary/10 border border-primary/20 rounded-xl animate-pulse">
            ...
            <span className="text-[13px] font-bold text-primary">
              {isTrigger ? "Fetching from LeadConnector..." : "Connecting to LeadConnector..."}
            </span>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 p-3 bg-success/10 border border-success/20 rounded-xl">
            ...
            <span className="text-[13px] font-bold text-success">
              {isTrigger
                ...
                : records.length > 1
                ? `Success! ${records.length} contacts processed in LeadConnector.`
                : "Success! Contact created in LeadConnector."}
            </span>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-destructive/10 border border-destructive/20 rounded-xl">
              ...
              <span className="text-[13px] font-bold text-destructive">
                {isTrigger
                  ? "No records found. Make sure there is data in LeadConnector for this event."
                  : "Failed. Please check your credentials and field mapping."}
              </span>
            </div>
            {isTrigger && onTestTrigger && (
              <button
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
              ...
              <p className="text-sm font-bold text-muted-foreground">
                {isTrigger && testStatus === "idle"
                  ? "Fetch a sample from LeadConnector using your configured pipeline and stage."
                  : "No data to display"}
              </p>
              {isTrigger && testStatus === "idle" && onTestTrigger && (
                <button
                  type="button"
                  onClick={onTestTrigger}
                  className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl ${ACCENT.bg} hover:bg-primary/90 text-primary-foreground text-[13px] font-bold shadow-sm transition-colors cursor-pointer`}
                >
